    ...
    class Noo_Check_Version {
        ...
        public function __construct( $product_id, $product_name, $api_url,
            ...
            if ( is_admin() ) {
                ...
                // }
                // Update number domain active with license.
                // add_action( 'admin_init', array( $this, 'update_licese_count' ) );
        ...
        public function add_license_settings_fields() {
            ...
            add_settings_field(
                $this->product_id . '-license-count',
                __( 'Number domain active with license', 'noo' ),
                array( $this, 'render_licese_count_settings_field' ),
                $settings_group_id,
                $settings_section_id
            );
        ...
        /**
         * Renders the limit license.
         */
        public function render_licese_count_settings_field() {
            $settings_field_name = $this->get_settings_field_name();
            $options = get_option( $settings_field_name );
            ?>
            <input type='text' name='<?php echo $settings_field_name; ?>[license_count]'
                   value='<?php echo $options['license_count']; ?>' class='regular-text'>
        <?php
        }
        ...
        /**
         * Update the limit license.
         */
        // public function update_licese_count() {
        //     $info = $this->get_license_info();
        //     if(!empty($info) && isset($info->number_of_activation_times)){
        //         update_option('license_count', $info->number_of_activation_times);
        //     }
        // }
        /**
        ...
        public function show_admin_notices() {
            ...
            if ( ! $options || ! isset( $options['email'] ) || ! isset( $options['license_key'] ) ||
                ...
                <div class="update-nag">
                    <?php echo $msg; ?> <a href="<?php echo admin_url( 'admin.php?page=' . $this->get_settings_page_slug() ); ?>">
        ...
        public function check_for_update( $transient ) {
            ...
            if ( $info !== false ) {
                if ( $this->is_theme() ) {
                    if(isset($info->number_of_activation_times)){
                        update_option('license_count', $info->number_of_activation_times);
                    }
                    // Theme update
